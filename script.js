    ...
    function openModal(id){
        ...
        (function(){
            ...
            function openModal(id){
                ...
                requestAnimationFrame(()=>{ box.classList.remove('scale-95','opacity-0','translate-y-4'); box.classList.add('scale-100','opacity-100','translate-y-0'); });
                previousFocus = document.activeElement; closeBtn && closeBtn.focus();
            }

            function closeModal(){
                box.classList.add('scale-95','opacity-0','translate-y-4'); box.classList.remove('scale-100','opacity-100','translate-y-0');
                setTimeout(()=>{ modal.classList.add('hidden'); modal.classList.remove('flex'); if(previousFocus) previousFocus.focus(); }, 250);
            }

            readmoreLinks.forEach(a=> a.addEventListener('click', e=>{ e.preventDefault(); const id = a.dataset.projectId; openModal(id); }));
